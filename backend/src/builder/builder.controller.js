...
const addBuilder = ({
  ...
  const { error } = validateBuilderCreateData(httpRequest.body);
...
throw new BadRequestError('Builder already exist!');
};
...
// Delete Builder
const deleteBuilder = ({ doDeleteBuilder,BadRequestError,doCheckProjectbyBuilder }) => async (
  httpRequest,
) => {
  const { id } = httpRequest.params;
  try {
    await doCheckProjectbyBuilder({
      cus_id:id,
    BadRequestError,
      });
  } catch (error) {
    const data = await doDeleteBuilder({
      id,
      BadRequestError,
    });
    return {
  ...
  throw new BadRequestError('Builder delete not found !');
  }
