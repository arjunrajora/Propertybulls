...
const doGetBannerPositionByPage = async ({ BadRequestError, BannerPosition, banner_location, page }) => {
  const whereClause = {
    page: page,
  };
  
  whereClause['$Advertisement.BannerPosition.banner_location$'] = {
    [Sequelize.Op.like]: `%${banner_location}%`
  };

  const GetData = await Advertisementsubscription.findOne({
    where: whereClause,
    include: {
      model: Advertisement,
      include: {
