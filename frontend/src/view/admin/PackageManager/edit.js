...
export default function PackageEdit() {
  ...
  useEffect(() => {
    const apI = config.API_URL + "subscription/" + id;
    fetch(apI, options)
      .then((response) => response.json())
      .then((value) => {
        console.log("kamal", value.data);
        setSubscription(value.data);
        setRole_id(value.data.role_id)
        setPackage_name(value.data.package_name);
        setPackage_validity(value.data.package_validity)
        setPackage_price(value.data.package_price);
        setPackage_discount(value.data.package_discount);
        setF1(value.data.f1);
        setF2(value.data.f2);
        setF3(value.data.f3);
        setF4(value.data.f4);
  ...
  const UpdatePackage = async (event) => {
    ...
    const apiUrl = config.API_URL + 'subscription/update/' + id;
                    ...
                    <div className="card-body">
                      ...
                      >
                        <div className="form-body">
                          <h2 className="form-section">EDIT PACKAGE</h2>
